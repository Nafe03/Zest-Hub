...
-- Professional Service fetching with safe cloneref
...
local function safeParentGui(gui, player)
    local ok = pcall(function() gui.Parent = GetSafeService("CoreGui") end)
...
--------------------------------------------------------------------------------
-- Premium Notification Object (OOP System)
--------------------------------------------------------------------------------
local NotifyObject = {}
NotifyObject.__index = NotifyObject

function NotifyObject.new(container, options, defaultThemeColor)
    local self = setmetatable({}, NotifyObject)
    self.Duration = options.Duration or 4
    local themeColor = options.Color or defaultThemeColor
    
    self.Frame = Instance.new("Frame")
    self.Frame.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
    self.Frame.Size = UDim2.new(1, 0, 0, 0) -- Starts at 0 height
    ...
    self.TextLabel.Size = UDim2.new(1, -24, 1, -20)
    self.TextLabel.Position = UDim2.new(0, 12, 0, 10)
    self.TextLabel.Font = Enum.Font.Gotham
    self.TextLabel.TextSize = 12
    self.TextLabel.TextColor3 = Color3.fromRGB(220, 220, 220)
    ...
    -- Format RGB to use in HTML tags for RichText
    local r, g, b = math.floor(themeColor.R * 255), math.floor(themeColor.G * 255), math.floor(themeColor.B * 255)
    local titleText = options.Title or "Notification"
    local bodyText = options.Text or ""
    
    self.TextLabel.Text = string.format("<b><font color='rgb(%d,%d,%d)'>%s</font></b>\n%s", r, g, b, titleText, bodyText)

    self:Show()
    return self
end

function NotifyObject:Show()
    smoothTween(self.Frame, {Size = UDim2.new(1, 0, 0, 65), BackgroundTransparency = 0}, 0.4)
    smoothTween(self.TextLabel, {TextTransparency = 0}, 0.4)
    smoothTween(self.Stroke, {Transparency = 0}, 0.4)
...
function NotifyObject:Destroy()
    local fadeOut = smoothTween(self.Frame, {Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1}, 0.3)
    smoothTween(self.TextLabel, {TextTransparency = 1}, 0.2)
    smoothTween(self.Stroke, {Transparency = 1}, 0.2)
...
--------------------------------------------------------------------------------
-- Main UI Library Creation Function
--------------------------------------------------------------------------------
function UILibrary.new(options)
    ...
    -- Dedicated GUI for Notifications so they show even if Main UI is toggled off
    local NotificationGui = Instance.new("ScreenGui")
    NotificationGui.Name = options.Name .. "_Notifications"
    NotificationGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    NotificationGui.ResetOnSpawn = false
    NotificationGui.DisplayOrder = 10005
    safeParentGui(NotificationGui, player)
    ...
    -- Built in Notify function
    function Window:Notify(notifOptions)
        return NotifyObject.new(NotificationContainer, notifOptions or {}, self.DefaultColor)
    end
                    ...
                    AddLabel = function(self, text, opts)
                        ...
                        local element = {
                            ...
                            AddColorPicker = function(elementSelf, id, pickerOptions)
                                ...
                                elementSelf.ColorPicker = picker
                                return picker
                            end
                        }
                        table.insert(self.Elements, element)
    ...
    -- Dynamic Theme Color Changing System
