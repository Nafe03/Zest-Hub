-- Complete Enhanced Script with CamLock
...
-- Get movement vector based on current inputs
local function getMovementVector()
    ...
    -- Up/Down
    if Keys.Space or Keys.Q then
        moveVector = moveVector + Vector3.new(0, 1, 0)
    end
    if Keys.LeftControl or Keys.E then
        moveVector = moveVector - Vector3.new(0, 1, 0)
    end
...
local function updateFreeCam(deltaTime)
    ...
    if moveVector.Magnitude > 0 then
        moveVector = moveVector.Unit * MOVE_SPEED * speedMultiplier * deltaTime
...
local function onMouseMoved()
    ...
    local deltaX = Mouse.X - (Camera.ViewportSize.X / 2)
    local deltaY = Mouse.Y - (Camera.ViewportSize.Y / 2)
    
    -- Update rotation
    CameraRotation = CameraRotation + Vector2.new(
        -deltaX * MOUSE_SENSITIVITY,
        math.clamp(-deltaY * MOUSE_SENSITIVITY, -math.pi/2 + 0.1, math.pi/2 - 0.1)
    )
    
    -- Keep horizontal rotation in bounds
    if CameraRotation.X > math.pi then
        CameraRotation = Vector2.new(CameraRotation.X - 2 * math.pi, CameraRotation.Y)
    elseif CameraRotation.X < -math.pi then
        CameraRotation = Vector2.new(CameraRotation.X + 2 * math.pi, CameraRotation.Y)
    end
end

-- Handle scroll wheel for zoom
local function onScrollWheel(actionName, inputState, inputObj)
    if not FreeCamEnabled or inputState ~= Enum.UserInputState.Change then return end
    
    local scrollDelta = inputObj.Position.Z
    CurrentFOV = math.clamp(CurrentFOV - (scrollDelta * ZOOM_SPEED), MIN_FOV, MAX_FOV)
end

-- Handle key input
local function onKeyChanged(key, isPressed)
...
local function onInputEnded(input, gameProcessed)
    ...
    local keyName = input.KeyCode.Name
    onKeyChanged(keyName, false)
end

-- Initialize free cam position and rotation
local function initializeFreeCam()
    -- Set initial position and rotation from current camera
    CameraPosition = Camera.CFrame.Position
    
    -- Extract rotation from current camera CFrame
    local cf = Camera.CFrame
    local x, y, z = cf:ToEulerAnglesYXZ()
    CameraRotation = Vector2.new(-y, x)
...
local function startFreeCam()
    ...
    -- Connect mouse movement (using Mouse.Move for better compatibility)
    MouseConnection = Mouse.Move:Connect(onMouseMoved)
...
-- Toggle free cam function (for your UI)
...
-- Emergency exit (in case mouse gets stuck)
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if input.KeyCode == Enum.KeyCode.Escape and FreeCamEnabled then
        stopFreeCam()
    end
end)
...
-- Inside the Silent Aim section, modified choosetarget function with prediction removed
...
-- Fixed Hitmarker Function
function runhitmark(v140)
if not allvars.hitmarkbool then return end

-- Create hitmarker with better error handling
local success, err = pcall(function()
    ...
    hit_img.Position = UDim2.new(0.5, -25, 0.5, -25)
    ...
    -- Animate hitmarker
    local fadeTime = allvars.hitmarkfade or 0.5
    ...
    -- Safety cleanup in case tween fails
    game:GetService("Debris"):AddItem(hitpart, fadeTime + 0.1)
end)
...
-- For tracers, modify the tracer creation in the aimmodfunc:
local function CreateTracer(startPos, endPos)
    ...
    if allvars.usebeamtracer then
        ...
        attachment2.Position = Vector3.new(0, 0, beam.Size.Z)
        
        local beamEffect = Instance.new("Beam", beam)
        beamEffect.Attachment0 = attachment1
        beamEffect.Attachment1 = attachment2
        beamEffect.Color = ColorSequence.new(allvars.tracercolor3)
        beamEffect.Width0 = allvars.tracerwidth
        beamEffect.Width1 = allvars.tracerwidth
        beamEffect.LightEmission = allvars.tracerbloom and 1 or 0
        beamEffect.Transparency = NumberSequence.new(allvars.tracertrans)
        ...
        connection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            beamEffect.Transparency = NumberSequence.new(allvars.tracertrans + (1 - allvars.tracertrans) * progress)
            beamEffect.Width0 = allvars.tracerwidth * (1 - progress)
            beamEffect.Width1 = allvars.tracerwidth * (1 - progress)
        ...
        game:GetService("Debris"):AddItem(beam, fadeTime + 0.1)
    else
...
-- Define the instant reload function
local function instrelMODfunc()
    -- Your instant reload logic here
    -- This is a placeholder - replace with actual instant reload functionality
...
WorldStuff:AddToggle('Remove SunRays', {
	...
	Callback = function(v)
	   if v then
		   game:GetService("Lighting").SunRays.Enable = false
		else
		    game:GetService("Lighting").SunRays.Enable = true
	    end
	end
})
...
-- Fixed Arms ForceField
WorldStuff:AddToggle("Arms ForceField", {
	Text = "Arms ForceField",
	Default = false,	
    Callback = function(value)
		if value then 
			-- Function to apply forcefield to arms
			...
			local connection
			connection = workspace.Camera.ChildAdded:Connect(function(child)
			...
			getgenv().armsForceFieldConnection = connection
			...
			if getgenv().armsForceFieldConnection then
				getgenv().armsForceFieldConnection:Disconnect()
				getgenv().armsForceFieldConnection = nil
			end
		end
	end
})
...
WorldStuff:AddLabel('Arms Color'):AddColorPicker('ArmsColor', {
    ...
    Title = 'Arms Color',
    Callback = function(Value)
        -- Check if arms are in ForceField and change their color
        local viewModel = workspace.Camera:FindFirstChild("ViewModel")
        if viewModel then
            -- Get all arm parts
            local armParts = {
            ...
            -- Change color if arms are in ForceField material
            for _, part in pairs(armParts) do
...
WorldStuff:AddToggle('Disable tree', {
    Text = 'Disable tree',
    Default = false,
    Tooltip = 'Disable tree',
    Callback = function(v)
        local trees = workspace.SpawnerZones.Foliage
        if v then
...
function performNetworkDesync()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Add your network desync logic here
        print("Performing network desync...")
    end
end
...
function applyFakePosition()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local rootPart = LocalPlayer.Character.HumanoidRootPart
        -- Add your fake position logic here
        print("Applying fake position...")
    end
end
...
uhhh:AddToggle('ActivateResolver', {
    ...
    Callback = function()
        if scriptloading or cfgloading then return end
        ...
        if tick() > aimresolvertime then
            ...
            -- Properly reset character
            hrp.Anchored = false
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.CanCollide = true
            
            -- Restore original position if needed
            if not allvars.desyncbool then
...
Others:AddToggle('dbjump', {
    Text = 'dbjump',
    Default = false,
    Tooltip = 'Enables dbjump',
    Callback = function(v)
        candbjump = v
    end
})
...
runs.Heartbeat:Connect(function(delta)
    ...
    if allvars.aimtrigger and aimtarget and aimtargetpart then
        local origin = localplayer.Character.HumanoidRootPart.Position + Vector3.new(0, 1.5, 0)
        if allvars.desyncbool and desynctable and desynctable[3] and desynctable[3].Position then
            origin = desynctable[3].Position + Vector3.new(0, 1.5, 0)
        end
        ...
        local raycastParams = RaycastParams.new()
        raycastParams.FilterType = Enum.RaycastFilterType.Blacklist
        raycastParams.FilterDescendantsInstances = {LocalPlayer.Character, aimtarget.Character}
        
        local raycastResult = workspace:Raycast(origin, (aimtargetpart.Position - origin).Unit * (aimtargetpart.Position - origin).Magnitude, raycastParams)
        
        if not raycastResult and fpsrequired and a1table then
            fpsrequired.action(a1table, true)
            task.wait()
            fpsrequired.action(a1table, false)
        end
    ...
    if charsemifly and localplayer.Character and ACBYPASS_SYNC == true then --semifly
        ...
        if uis:IsKeyDown(Enum.KeyCode.W) then
            dir = dir + wcamera.CFrame.LookVector
        elseif uis:IsKeyDown(Enum.KeyCode.S) then
            dir = dir - wcamera.CFrame.LookVector
        end

        if uis:IsKeyDown(Enum.KeyCode.A) then
            dir = dir - wcamera.CFrame.RightVector
